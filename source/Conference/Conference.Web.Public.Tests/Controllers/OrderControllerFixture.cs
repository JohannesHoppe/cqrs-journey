...
namespace Conference.Web.Public.Tests.Controllers.OrderControllerFixture
{
    public class given_controller
    {
        ...
        public given_controller()
        {
            ...
            sut = new OrderController(Mock.Of<IConferenceDao>(), orderDao.Object, bus.Object);

            var routeData = new RouteData();
            routeData.Values.Add("conferenceCode", "conference");

            sut.ControllerContext = Mock.Of<ControllerContext>(x => x.RouteData == routeData);
        }
        ...
        {
            // Act
            var result = (RedirectToRouteResult) sut.Display(Guid.NewGuid());
        ...
        {
            // Act
            var result = (ViewResult) sut.Find();
        ...
        public void when_find_order_with_valid_email_and_access_code_then_redirects_to_display_with_order_id()
        {
            ...
            orderDao.Setup(r => r.LocateOrder("info@contoso.com", "asdf")).Returns(orderId);

            // Act
            var result = (RedirectToRouteResult) sut.Find("info@contoso.com", "asdf");
    ...
    {
        private readonly OrderSeats orderSeats;
        ...
        {
            // Act
            var result = (ViewResult) sut.AssignSeats(orderSeats.OrderId);

            // Assert
            Assert.NotNull(result);
            Assert.Equal(orderSeats, result.Model);
        }
        ...
        {
            // Act
            var result = (RedirectToRouteResult) sut.AssignSeats(Guid.NewGuid());
        ...
        {
            // Act
            var result = (RedirectToRouteResult) sut.AssignSeats(Guid.NewGuid(), new List<OrderSeat>());
        ...
        {
            // Act
            var result = (RedirectToRouteResult) sut.AssignSeats(orderSeats.OrderId, new List<OrderSeat> {
                new OrderSeat(1, "Precon") {
                    Attendee = new PersonalInfo {
                        Email = "a@a.com",
                        FirstName = "A",
            ...
            var cmd = commands.OfType<AssignSeat>().Single();
            Assert.Equal(orderSeats.AssignmentsId, cmd.SeatAssignmentsId);
        ...
        public void when_previously_assigned_seat_email_becomes_null_then_sends_unassign_command()
        {
            ...
            var cmd = commands.OfType<UnassignSeat>().Single();
            Assert.Equal(orderSeats.AssignmentsId, cmd.SeatAssignmentsId);
