...
namespace Conference
{
    ...
    public class ConferenceService
    {
        ...
        public ConferenceService(IEventBus eventBus, string nameOrConnectionString = "ConferenceManagement")
        {
            ...
            retryPolicy = new RetryPolicy<SqlAzureTransientErrorDetectionStrategy>(new Incremental(5, TimeSpan.FromSeconds(0), TimeSpan.FromSeconds(1.5)) {FastFirstRetry = true});
        ...
        public void CreateConference(ConferenceInfo conference)
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                var existingSlug = retryPolicy.ExecuteAction(() =>
                    context.Conferences
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                var conference = retryPolicy.ExecuteAction(() => context.Conferences.Find(conferenceId));
                if (conference == null) {
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                return retryPolicy.ExecuteAction(() => context.Conferences.FirstOrDefault(x => x.Slug == slug));
            }
        }
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                return retryPolicy.ExecuteAction(() => context.Conferences.FirstOrDefault(x => x.OwnerEmail == email && x.AccessCode == accessCode));
            }
        }
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                return retryPolicy.ExecuteAction(() => context.Seats.Find(seatTypeId));
            }
        }
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                return retryPolicy.ExecuteAction(() => context.Orders.Include("Seats.SeatInfo")
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                var existing = retryPolicy.ExecuteAction(() => context.Conferences.Find(conference.Id));
                if (existing == null) {
                    throw new ObjectNotFoundException();
                }
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                var existing = retryPolicy.ExecuteAction(() => context.Seats.Find(seat.Id));
                if (existing == null) {
                    throw new ObjectNotFoundException();
                }
                ...
                // (and therefore is not published either).
                if (retryPolicy.ExecuteAction(() => context.Conferences.Where(x => x.Id == conferenceId).Select(x => x.WasEverPublished).FirstOrDefault())) {
        ...
        {
            using (var context = new ConferenceContext(nameOrConnectionString)) {
                var seat = retryPolicy.ExecuteAction(() => context.Seats.Find(id));
                if (seat == null) {
                    throw new ObjectNotFoundException();
                }

                var wasPublished = retryPolicy.ExecuteAction(() => context.Conferences
