...
namespace Registration
{
    ...
    ///     <para>
    ///         This entity does not implement the <see cref="IMementoOriginator" /> interface because we do not expect each
    ///         instance
    ///         to have a long event stream, nor each instance will be accessed very frequently and from the same process to
    ///         benefit from in-memory caching.
    ///     </para>
    /// </remarks>
    public class Order : EventSourced
    {
        ...
        /// <summary>
        ///     Mapping old version of the <see cref="OrderPaymentConfirmed" /> event to the new version (
        ///     <see cref="OrderConfirmed" />).
        ...
        protected Order(Guid id)
            : base(id)
        {
            Handles<OrderPlaced>(OnOrderPlaced);
            Handles<OrderUpdated>(OnOrderUpdated);
            Handles<OrderPartiallyReserved>(OnOrderPartiallyReserved);
            Handles<OrderReservationCompleted>(OnOrderReservationCompleted);
            Handles<OrderExpired>(OnOrderExpired);
            Handles<OrderPaymentConfirmed>(e => OnOrderConfirmed(Mapper.Map<OrderConfirmed>(e)));
            Handles<OrderConfirmed>(OnOrderConfirmed);
            Handles<OrderRegistrantAssigned>(OnOrderRegistrantAssigned);
            Handles<OrderTotalsCalculated>(OnOrderTotalsCalculated);
        }
            ...
            : this(id)
        {
            ...
            Update(new OrderPlaced {
                ConferenceId = conferenceId,
                Seats = all,
                ReservationAutoExpiration = DateTime.UtcNow.Add(ReservationAutoExpiration),
                AccessCode = HandleGenerator.Generate(6)
            });
            Update(new OrderTotalsCalculated {Total = totals.Total, Lines = totals.Lines != null ? totals.Lines.ToArray() : null, IsFreeOfCharge = totals.Total == 0m});
        }
        ...
        public void UpdateSeats(IEnumerable<OrderItem> items, IPricingService pricingService)
        {
            ...
            Update(new OrderUpdated {Seats = all});
            Update(new OrderTotalsCalculated {Total = totals.Total, Lines = totals.Lines != null ? totals.Lines.ToArray() : null, IsFreeOfCharge = totals.Total == 0m});
        }
        ...
        public void MarkAsReserved(IPricingService pricingService, DateTime expirationDate, IEnumerable<SeatQuantity> reservedSeats)
        {
            ...
            if (seats.Any(item => item.Quantity != 0 && !reserved.Any(seat => seat.SeatType == item.SeatType && seat.Quantity == item.Quantity))) {
                var totals = pricingService.CalculateTotal(conferenceId, reserved.AsReadOnly());

                Update(new OrderPartiallyReserved {ReservationExpiration = expirationDate, Seats = reserved.ToArray()});
                Update(new OrderTotalsCalculated {Total = totals.Total, Lines = totals.Lines != null ? totals.Lines.ToArray() : null, IsFreeOfCharge = totals.Total == 0m});
            } else {
                Update(new OrderReservationCompleted {ReservationExpiration = expirationDate, Seats = reserved.ToArray()});
            }
        }
        ...
        public void Expire()
        {
            ...
            Update(new OrderExpired());
        }

        public void Confirm()
        {
            Update(new OrderConfirmed());
        }

        public void AssignRegistrant(string firstName, string lastName, string email)
        {
            Update(new OrderRegistrantAssigned {
                FirstName = firstName,
                LastName = lastName,
                Email = email
            });
        ...
        public SeatAssignments CreateSeatAssignments()
        {
            ...
            return new SeatAssignments(Id, seats.AsReadOnly());
        }
