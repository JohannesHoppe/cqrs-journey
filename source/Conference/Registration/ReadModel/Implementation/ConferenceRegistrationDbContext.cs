...
namespace Registration.ReadModel.Implementation
{
    ...
    public class ConferenceRegistrationDbContext : DbContext
    {
        ...
        {
            retryPolicy = new RetryPolicy<SqlAzureTransientErrorDetectionStrategy>(new Incremental(3, TimeSpan.FromSeconds(0), TimeSpan.FromSeconds(1.5)) {FastFirstRetry = true});
        ...
        public T Find<T>(Guid id)
            where T : class
        {
            return retryPolicy.ExecuteAction(() => Set<T>().Find(id));
        }
            ...
            where T : class
        {
            ...
            retryPolicy.ExecuteAction(() => SaveChanges());
        }
    }
}