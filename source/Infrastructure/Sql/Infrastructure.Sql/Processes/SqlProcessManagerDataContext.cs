...
{
    /// <summary>
    ///     Data context used to persist instances of <see cref="IProcessManager" /> (also known as Sagas in the CQRS
    ///     community) using Entity Framework.
    /// </summary>
    /// <typeparam name="T">The entity type to persist.</typeparam>
    /// <remarks>
    ///     <para>
    ///         See <see cref="http://go.microsoft.com/fwlink/p/?LinkID=258564">Reference 6</see> for a description of what
    ///         is a Process Manager.
    ///     </para>
    ///     <para>
    ///         This is a very basic implementation, and would benefit from several optimizations.
    ///         For example, it would be very valuable to provide asynchronous APIs to avoid blocking I/O calls.
    ///         It would also benefit from dispatching commands asynchronously (but in a resilient way), similar to what the
    ///         EventStoreBusPublisher does.
        ...
        where T : class, IProcessManager
    {
        ...
        public SqlProcessManagerDataContext(Func<DbContext> contextFactory, ICommandBus commandBus, ITextSerializer serializer)
        {
            ...
            retryPolicy = new RetryPolicy<SqlAzureTransientErrorDetectionStrategy>(new Incremental(3, TimeSpan.Zero, TimeSpan.FromSeconds(1)) {FastFirstRetry = true});
            ...
            if (undispatched != null) {
                ...
                try {
                    ...
                    if (originalCommandsCount != deserializedCommands.Count) {
                        ...
                        undispatched.Commands = serializer.Serialize(deserializedCommands);
                        try {
                            context.SaveChanges();
                ...
                context.Set<UndispatchedMessages>().Remove(undispatched);
        ...
        public T Find(Expression<Func<T, bool>> predicate, bool includeCompleted = false)
        {
            ...
            if (!includeCompleted) {
                ...
                pm = retryPolicy.ExecuteAction(() =>
                    context.Set<T>().Where(predicate.And(x => x.Completed == false)).FirstOrDefault());
            }
            ...
            if (pm != null) {
                ...
                var undispatchedMessages = context.Set<UndispatchedMessages>().Find(pm.Id);
                try {
                    ...
                    context.Entry(undispatchedMessages).Reload();

                    undispatchedMessages = context.Set<UndispatchedMessages>().Find(pm.Id);
