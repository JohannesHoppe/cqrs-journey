...
{
    /// <summary>
    ///     This is an extremely basic implementation of the event store (straw man), that is used only for running the sample
    ///     application
    ///     without the dependency to the Windows Azure Service Bus when using the DebugLocal solution configuration.
    ///     It does not check for event versions before committing, nor is transactional with the event bus nor resilient to
    ///     connectivity errors or crashes.
    ///     It does not do any snapshots either for entities that implement <see cref="IMementoOriginator" />, which would
    ///     benefit the usage of SeatsAvailability.
    /// </summary>
