...
namespace Infrastructure.Azure.EventSourcing
{
    ...
    ///     <para>
    ///         It supports caching of snapshot if the entity implements the <see cref="IMementoOriginator" /> interface.
    ///         Ideally, this could be optimized to
    ///         integrate better with sessionful message processors, so that the when the process is guaranteed to be the
    ///         single temporary writer of a certain entity
    ///         instance, then there is no need to check for updates in the the <see cref="IEventStore" /> since the last
    ///         cached snapshot (<see cref="IMemento" />).
    ///     </para>
    ///     <para>Also, it would be very valuable to provide asynchronous APIs to avoid blocking I/O calls.</para>
    ///     <para>
    ///         See <see cref="http://go.microsoft.com/fwlink/p/?LinkID=258557"> Journey chapter 7</see> for more potential
        ...
        where T : class, IEventSourced
    {
        ...
        public AzureEventSourcedRepository(IEventStore eventStore, IEventStoreBusPublisher publisher, ITextSerializer serializer, IMetadataProvider metadataProvider, ObjectCache cache)
        {
            ...
            if (typeof(IMementoOriginator).IsAssignableFrom(typeof(T)) && this.cache != null) {
                ...
                originatorEntityFactory = (id, memento, events) => (T) mementoConstructor.Invoke(new object[] {id, memento, events});
                cacheMementoIfApplicable = originator => {
                    var key = GetPartitionKey(originator.Id);
                ...
                getMementoFromCache = id => (Tuple<IMemento, DateTime?>) this.cache.Get(GetPartitionKey(id));
                markCacheAsStale = id => {
                ...
                cacheMementoIfApplicable = o => { };
                getMementoFromCache = id => { return null; };
                markCacheAsStale = id => { };
            }
        }
        ...
        {
            var cachedMemento = getMementoFromCache(id);
            if (cachedMemento != null && cachedMemento.Item1 != null) {
                ...
                if (!cachedMemento.Item2.HasValue || cachedMemento.Item2.Value < DateTime.UtcNow.AddSeconds(-1)) {
                    deserialized = eventStore.Load(GetPartitionKey(id), cachedMemento.Item1.Version + 1).Select(Deserialize);
                } else {
                ...
                return originatorEntityFactory.Invoke(id, cachedMemento.Item1, deserialized);
            } else {
                var deserialized = eventStore.Load(GetPartitionKey(id), 0)
                    .Select(Deserialize)
        ...
        public void Save(T eventSourced, string correlationId)
        {
            ...
            publisher.SendAsync(partitionKey, events.Length);

            cacheMementoIfApplicable.Invoke(eventSourced);
        }
    }
}